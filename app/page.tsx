...
export function TrackerPage({ view = 'home' }: { view?: TrackerView }) {
  ...
  const filtered = useMemo(() => [...pipelines].filter((item) => {
    ...
    const haystack = `${item.name} ${(item.topics ?? []).join(' ')} ${institution.label} ${item.summary} ${item.input} ${item.output} ${item.representation} ${item.simulator} ${hardware.label} ${hardware.facets.join(' ')} ${tasks.join(' ')} ${locomotion.join(' ')} ${bodyScope.join(' ')}`.toLowerCase();
  ...
  return (
    <main id="top" className={view === 'capabilities' ? 'capabilities-page' : view === 'pipelines' ? 'pipelines-page' : undefined}>
      ...
      <section className="catalog-section" id="catalog">
        <div className="section-heading"><div><p className="eyebrow">PIPELINE ATLAS</p><h2>Compare full systems.</h2></div><p>Robotics, graphics inverse rendering and relighting, physical simulation, static-scene reconstruction, and every method in the Historical Lineage are expanded into connected modules. Scene counts preserve the source unit; N/A means the method does not convert scenes. “Latest” is not inherently better.</p></div>
        ...
        <div className={`filter-bar${filtersPinned ? ' filter-open' : ''}`} aria-label="Pipeline filters">
          ...
          <div className="stage-multi-filter" aria-label="Search graphics and simulation topics"><span>TOPIC SEARCH</span>{['Inverse rendering', 'Relighting', 'Physical simulation'].map((topic) => <button type="button" key={topic} aria-pressed={query === topic} className={query === topic ? 'active' : ''} onClick={() => setQuery(query === topic ? '' : topic)}>{topic}</button>)}</div>
        ...
        <div className="pipeline-list">
          {displayedPipelines.map((item, index) => {
            ...
            <summary>
              ...
              <span className="row-name"><strong>{item.name}</strong><small>{item.date} · {item.domain}</small>{item.topics && <small>{item.topics.join(' · ')}</small>}<span className={`row-institution ${institution.verified ? 'institution-verified' : ''}`}>INSTITUTE · {institution.label}</span>{sceneReport && <span className={`row-scene-count scene-${sceneReport.status}`}>SCENES · {sceneReport.amount}</span>}</span>
